                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-2xs overflow-hidden dark:bg-neutral-900 dark:border-neutral-700">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                        ...
                        <tr>
                            ...
                            <th wire:click="sortBy('activity')" scope="col" class="cursor-pointer px-6 py-3 text-start">
                                <div class="flex items-center gap-x-2 text-xs font-semibold uppercase text-gray-800 dark:text-neutral-200">
                                    Activité
                                    <x-flux::icon.sort field='activity' class="size-3.5" direction='{{ $sortDirection }}' sortField='{{ $sortField }}' />
                                </div>
                            </th>
                                        ...
                                        <flux:menu>
                                            <div class="p-1 space-y-0.5">
                                                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700"
                                                    href="{{ route('admin.entreprises.edit', ['entreprise' => $product->id]) }}">
                                                    Modifier
                                                </a>

                                                <form method="POST" action="{{ route('admin.entreprises.delete', ['entreprise' => $product->id]) }}" class="w-full">
                                                    @csrf
                                                    @method('DELETE')
